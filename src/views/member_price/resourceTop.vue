...
<script>
import { resourceTopList,resourceTopInfo,resourceTopAdd,resourceTopUpdate,resourceTopDel } from "@/api/study/member_price";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      resourceTopList(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      resourceTopInfo(id).then(response => {
    ...
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != 0) {
            resourceTopUpdate(this.form).then(response => {
            ...
            this.form.type=2;
            resourceTopAdd(this.form).then(response => {
              if (response.code === 200) {
